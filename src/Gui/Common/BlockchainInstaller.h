...
private:
  ...
  const QDir m_lithiumbitDir;
  const QDir m_applicationDir;

  bool checkIfCurrentBlockchainExists() const;
  bool checkIfLithiumBitBlockchainExists() const;
  bool getGenesisBlockFromBlockchain(char** _genesisBlockData) const;
  bool checkIfBlockchainOutdated(quint64& _current, quint64& _lithiumbit) const;
  QFileInfo currentBlockchainInfo() const;
  QFileInfo lithiumbitBlockchainInfo() const;
