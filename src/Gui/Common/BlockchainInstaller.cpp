...
BlockchainInstaller::BlockchainInstaller(QObject* _parent) : QObject(_parent), m_blockIndexesFileName("blockindexes.dat"), m_blocksFileName("blocks.dat"),
  m_lithiumbitDir(Settings::instance().getDataDir().absolutePath()), m_applicationDir(QDir::current()) {
}
...
void BlockchainInstaller::exec() {
  ...
  if (!checkIfLithiumBitBlockchainExists()) {
    installBlockchain();
    return;
  }

  quint64 currentHeight;
  quint64 lithiumbitHeight;
  if (!checkIfBlockchainOutdated(currentHeight, lithiumbitHeight)) {
    return;
  }
  ...
  QuestionDialog dlg(tr("Blockchain installation"), QString(questionStringTemplate).arg(lithiumbitHeight).arg(currentHeight), nullptr);
...
bool BlockchainInstaller::checkIfLithiumBitBlockchainExists() const {
  return m_lithiumbitDir.exists() && m_lithiumbitDir.exists(m_blocksFileName);
}
...
bool BlockchainInstaller::checkIfBlockchainOutdated(quint64& _sourceHeight, quint64& _targetHeight) const {
  ...
  QFile targetBlockIndexesFile(m_lithiumbitDir.absoluteFilePath(m_blockIndexesFileName));
...
QFileInfo BlockchainInstaller::lithiumbitBlockchainInfo() const {
  return QFileInfo(m_lithiumbitDir.absoluteFilePath(m_blocksFileName));
}
...
void BlockchainInstaller::installBlockchain() {
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blockIndexesFileName),
    m_lithiumbitDir.absoluteFilePath(m_blockIndexesFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
  ...
  Q_EMIT copyFileSignal(m_applicationDir.absoluteFilePath(m_blocksFileName), m_lithiumbitDir.absoluteFilePath(m_blocksFileName));
  if (waitLoop.exec() != 0) {
    workerThread.quit();
    workerThread.wait();
    return;
